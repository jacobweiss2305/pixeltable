...
def init(
    ...
    if messages_table_name not in pxt.list_tables():
        ...
        messages_table = pxt.create_table(
            ...
            schema_or_df={'role': pxt.String, 'content': pxt.String, 'timestamp': pxt.Timestamp},
            if_exists='replace',
        )
    else:
    ...
    if agent_name not in pxt.list_tables():
        agent_table = pxt.create_table(path_str=agent_name, schema_or_df={'prompt': pxt.String}, if_exists='replace')

        setup_agent_table(agent_table, agent_tools, system_prompt, messages_table, model_name)
...
def create_messages_with_history(
    past_context: List[Dict], system_prompt: str, current_prompt: str, injected_messages: List[Dict] = None
) -> List[Dict]:
...
def setup_agent_table(
    agent_table: pxt.Table, agent_tools: pxt.tools, system_prompt: str, messages_table: pxt.Table, model_name: str
):
    ...
    agent_table.add_computed_column(messages_context=fetch_messages(), if_exists='replace')
    ...
    if agent_tools is not None:
        ...
        agent_table.add_computed_column(
            tool_response=openai.chat_completions(model=model_name, messages=agent_table.messages, tools=agent_tools),
            if_exists='replace',
        )

        agent_table.add_computed_column(
            tool_result=openai.invoke_tools(agent_tools, agent_table.tool_response), if_exists='replace'
        )

        agent_table.add_computed_column(
            interpret_tool_result=create_prompt(agent_table.prompt, agent_table.tool_result), if_exists='replace'
        )
    ...
    agent_table.add_computed_column(
        final_response=openai.chat_completions(model=model_name, messages=result_messages), if_exists='replace'
    )

    agent_table.add_computed_column(answer=agent_table.final_response.choices[0].message.content, if_exists='replace')
...
def batch_inject_messages(messages_table, messages_to_inject: List[Dict]):
    ...
    messages_table.insert(
        [{'role': msg['role'], 'content': msg['content'], 'timestamp': current_time} for msg in messages_to_inject]
    )
